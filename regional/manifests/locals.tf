...
locals {
  ...
  kubernetes_monitor_templates = {
    "crash-loop-backoff" = {
      ...
      name                = "[Kubernetes] Pods in CrashLoopBackOff - namespace: {{kube_namespace.name}} cluster: ${local.kubernetes_cluster_name}"
      priority            = 3
    ...
    "failing-deployment-replicas" = {
      ...
      name                = "[Kubernetes] Deployments Replica Pods are down - cluster: ${local.kubernetes_cluster_name}"
      priority            = 3
    ...
    "failing-pods" = {
      ...
      name                = "[Kubernetes] Failed Pods in Namespaces - cluster: ${local.kubernetes_cluster_name}"
      priority            = 3
    ...
    "failing-statefulset-replicas" = {
      ...
      name                = "[Kubernetes] Statefulset Replicas are down - cluster: ${local.kubernetes_cluster_name}"
      priority            = 3
    ...
    "high-cpu-usage" = {
      ...
      name                = "[Kubernetes] Pod CPU usage is high - cluster: ${local.kubernetes_cluster_name}"
      priority            = 3
    ...
    "high-memory-usage" = {
      ...
      name                = "[Kubernetes] Pod memory usage is high - cluster: ${local.kubernetes_cluster_name}"
      priority            = 3
    ...
    "image-pull-backoff" = {
      message = <<-EOF
      Pod {{pod_name.name}} is in ImagePullBackOff on {{kube_namespace.name}} on cluster: {{kube_cluster_name.name}}.
      EOF

      name                = "[Kubernetes] Pods in ImagePullBackOff - namespace: {{kube_namespace.name}} cluster ${local.kubernetes_cluster_name}"
      priority            = 3
    ...
    "restarting-pods" = {
      ...
      name                = "[Kubernetes] Pods Restarting - cluster: ${local.kubernetes_cluster_name}"
      priority            = 3
    ...
    "unschedulable-node" = {
      ...
      name                = "[Kubernetes] Unschedulable Nodes - cluster: ${local.kubernetes_cluster_name}"
      priority            = 3
